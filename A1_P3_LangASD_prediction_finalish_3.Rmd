...
rmse(train$CHI_MLU, predict(big_model_new))

#interval TRAIN
int_TR_simple <- predictInterval(simple, train, level = .95) %>%
  cbind(train) %>%
  rownames_to_column()
...
# plotting TR_simple
int_TR_big %>%
  ggplot(aes(x = VISIT, y = fit, ymin = lwr, ymax = upr, color = Diagnosis)) +
  geom_point() +
...
int_TR_simple %>%
  ...
  geom_point() +
  geom_linerange() +
  labs(title = "predictions of",
       x="Participant", 
       y="Prediction w/ 95% PI") + 
  theme_bw()

#plotting big 
int_TR_big %>%
  ggplot(aes(x=1:352, y=fit, ymin=lwr, ymax=upr)) +
  geom_point() +
  geom_linerange() +
  labs(title = "",
       x="Participant", 
...
big_rmse <- Metrics::rmse(test_2$CHI_MLU, predict(big_model_new, newdata = test_2, allow.new.levels = T))
...
interval_big <- predictInterval(big_model_new, test_2, level = .95)
```
